...
在 `demo/` 目录下执行脚本，直接传入 **内核子系统 + 不利 Case** 参数即可。脚本会自动识别全局配置中的数据库类型，并在注入前自动验证数据库连接。
...
```bash
./demo.sh session max_connection -mode conn_storm -count 200
```
...
```bash
./demo.sh session max_connection -mode conn_exhaustion
```
...
```bash
./demo.sh session max_connection -mode thread_saturation -count 32
```
![线程池饱和](../static/demo/thread_saturation.svg)

#### D. 长事务持锁 (Long Transaction Lock Holding)

模拟长事务持锁对事务与并发控制路径的影响。

```bash
./demo.sh txn uncommitted_txn -table bmsql_stock -holders 2 -rows 500
```
...
```bash
./demo.sh sql plan_flip -threads 16 -interval 60000
```
...
- **参数形式**: 传入参数需符合当前 CLI 结构，即 `<SUBSYSTEM> <CASE> [OPTIONS]`，例如 `session max_connection -mode conn_storm`。
