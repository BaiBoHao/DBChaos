...
for graph in $SIMPLE_GRAPHS ; do
	...
	out=$(sed -e "s/@WIDTH@/${WIDTH}/g" \
		  ...
		  -e "s/@SKIP@/${SKIP}/g" \
		  <~/benchmarksql-5.0/run/misc/${graph}.R | R --no-save)
	if [ $? -ne 0 ] ; then
		echo "ERROR"
		echo "$out" >&2
		exit 3
...
for fname in ./data/blk_*.csv ; do
	...
	out=$(sed -e "s/@WIDTH@/${WIDTH}/g" \
		  ...
		  -e "s/@POINTSIZE@/${POINTSIZE}/g" \
		  -e "s/@SKIP@/${SKIP}/g" \
		  -e "s/@DEVICE@/${devname}/g" <~/benchmarksql-5.0/run/misc/blk_device_iops.R | R --no-save)
	if [ $? -ne 0 ] ; then
		echo "ERROR"
		echo "$out" >&2
		exit 3
	...
	out=$(sed -e "s/@WIDTH@/${WIDTH}/g" \
		  ...
		  -e "s/@POINTSIZE@/${POINTSIZE}/g" \
		  -e "s/@SKIP@/${SKIP}/g" \
		  -e "s/@DEVICE@/${devname}/g" <~/benchmarksql-5.0/run/misc/blk_device_kbps.R | R --no-save)
	if [ $? -ne 0 ] ; then
		echo "ERROR"
		echo "$out" >&2
		exit 3
...
for fname in ./data/net_*.csv ; do
	...
	out=$(sed -e "s/@WIDTH@/${WIDTH}/g" \
		  ...
		  -e "s/@POINTSIZE@/${POINTSIZE}/g" \
		  -e "s/@SKIP@/${SKIP}/g" \
		  -e "s/@DEVICE@/${devname}/g" <~/benchmarksql-5.0/run/misc/net_device_iops.R | R --no-save)
	if [ $? -ne 0 ] ; then
		echo "ERROR"
		echo "$out" >&2
		exit 3
	...
	out=$(sed -e "s/@WIDTH@/${WIDTH}/g" \
		  ...
		  -e "s/@POINTSIZE@/${POINTSIZE}/g" \
		  -e "s/@SKIP@/${SKIP}/g" \
		  -e "s/@DEVICE@/${devname}/g" <~/benchmarksql-5.0/run/misc/net_device_kbps.R | R --no-save)
	if [ $? -ne 0 ] ; then
		echo "ERROR"
		echo "$out" >&2
		exit 3
